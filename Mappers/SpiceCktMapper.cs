            ...
            if (module.Submodules.ContainsKey("Inductor"))
            {
                ...
                for (int i = 0; i != inductors.Count; i++)
                {
                    ...
                    var plusName = plus == inIndex
                        ...
                        ? "0"
                        : $"_{plus}";
                    ckt.Add(new Inductor($"I{i}", plusName, minusName, Convert.ToDouble(inductor.Features["Inductance"]) * 1e-3));
                }
            }
            ...
            if (module.Submodules.ContainsKey("Capacitor"))
            {
                ...
                for (int i = 0; i != capacitors.Count; i++)
                {
                    ...
                    var plusName = plus == inIndex
                        ...
                        ? "0"
                        : $"_{plus}";
                    ckt.Add(new Capacitor($"C{i}", plusName, minusName, Convert.ToDouble(capacitor.Features["Capacity"]) * 1e-9));
                }
            }
